 ...
 */
public class YoutubeCategories implements Serializable {
    String id,title,title_ar,title_fr,image;
    ArrayList<VideoChanel> chanels;
    ArrayList<VideoChanel> chanels_all;

    YoutubeCategories(JSONObject jsonObject){
        chanels=new ArrayList<>();
        try {
            id=jsonObject.getString("id");
            title=jsonObject.getString("title");
            title_ar=jsonObject.getString("title");
            title_fr=jsonObject.getString("title");
            ...
            for(int i=0;i<jsonObject.getJSONArray("chanels").length();i++){
                ...
                VideoChanel chanel=new VideoChanel(jsonObject2,"0");
                chanel.parent_id = id;
                chanels.add(chanel);
            }
            chanels_all = chanels;
